...
public:
   ...
   * robot. */
  geometry_msgs::Pose2D detectHalfCircle(cv::Mat &image, std::vector<cv::Point2f>& edgePositions);
...
private:
  ...
  /** @brief Computes how much of the circle given is actually present in the
    ...
    * present.
    * @param semiCircleStart starting angle of the semi-circle to be verified [rad]
